...
def make_binary_los_class(los_days: float) -> str:
    # Practical binary setup: short stay is 2 days or less, longer stay is over 2 days.
    if los_days <= 2:
        return "short"
    return "longer"
...
def prepare_modeling_data() -> tuple[pd.DataFrame, pd.Series, pd.Series]:
    ...
    df["los_class"] = df["los"].map(make_binary_los_class)
...
def evaluate_classification(
    y_true: pd.Series, y_pred: np.ndarray, y_score: np.ndarray | None = None
) -> dict[str, float]:
    metrics = {
        "accuracy": accuracy_score(y_true, y_pred),
        "precision_short": precision_score(y_true, y_pred, pos_label="short", zero_division=0),
        "recall_short": recall_score(y_true, y_pred, pos_label="short", zero_division=0),
        "f1_short": f1_score(y_true, y_pred, pos_label="short", zero_division=0),
        "macro_f1": f1_score(y_true, y_pred, average="macro", zero_division=0),
        "weighted_f1": f1_score(y_true, y_pred, average="weighted", zero_division=0),
    }
    if y_score is not None:
        y_true_bin = (y_true == "short").astype(int)
        metrics["roc_auc_short"] = roc_auc_score(y_true_bin, y_score)
    return metrics
...
def main() -> None:
    ...
    for model_name, estimator in models:
        ...
        if hasattr(pipeline, "predict_proba"):
            ...
            score = proba[:, short_idx]

        results.append({"model": model_name, **evaluate_classification(y_test, pred, score)})
        reports[model_name] = pd.DataFrame(
            classification_report(
                y_test, pred, labels=CLASS_LABELS, output_dict=True, zero_division=0
            )
        ).T
    ...
    results_df = pd.DataFrame(results).sort_values(
        ["f1_short", "roc_auc_short", "accuracy"], ascending=False
    ).reset_index(drop=True)
    results_path = OUTPUT_DIR / "binary_short_vs_longer_results.csv"
    results_df.to_csv(results_path, index=False)

    for model_name, report_df in reports.items():
        safe_name = model_name.lower().replace(" ", "_")
        report_df.to_csv(OUTPUT_DIR / f"{safe_name}_binary_short_vs_longer_report.csv")
        confusion_frames[model_name].to_csv(
            OUTPUT_DIR / f"{safe_name}_binary_short_vs_longer_confusion_matrix.csv"
        )

    class_dist = y.value_counts(normalize=True).reindex(CLASS_LABELS).fillna(0).mul(100)
    print("\nTarget Definition")
    print("short: LOS <= 2 days")
    print("longer: LOS > 2 days")
